...
stats = [("160+ 人", "累计独立用户", "去重 player_id，剔除测试账号"),
         ("47 人", "实际下场玩过的玩家", "其余为仅浏览/登录未开局"),
...
text(s, Inches(1.25), Inches(4.4), Inches(10.9), Inches(2.3),
     [[("从数据里读到的四件事", 14, ACCENT2, True)],
      [("", 7, MUTED, False)],
      [("① 双端适配真的有人在用 ", 12, TEXT, True),
       ("— 移动端 UA 产生 701 条事件、桌面端 1,295 条（移动端占 35%），「手机+电脑都能玩」不是宣传语。", 11.5, MUTED, False)],
      [("② 复活机制是被真实使用的核心循环 ", 12, TEXT, True),
       ("— 56 次复活中，看广告 38% / 花钻石 25% / 用令牌 21% / 好友助力 16%，四条路径都有人走。", 11.5, MUTED, False)],
      [("③ 武器使用率 92% 集中在手枪 ", 12, TEXT, True),
       ("— 133 次开局中手枪占 122 次；手枪是免费初始武器，说明多数玩家没走到解锁新武器的波次就阵亡了。", 11.5, MUTED, False)],
      [("④ 皮肤占商店购买的 71%", 12, TEXT, True),
       ("— 31 笔购买中 22 笔是皮肤（烈焰 7 / 虚空 4 / 黄金 4 / 霓虹 3 / 暗影 2）。这是本次皮肤改造最直接的正向验证。", 11.5, MUTED, False)],
      [("", 7, MUTED, False)],
      [("数据诚实性：", 10.5, AMBER, True),
       ("死亡波次分布中「第 1 波 109 次」包含开发与自动化测试数据，不能当作真实难度曲线；场均时长同受测试会话影响。", 10.5, DIM, False)],
      [("用户数口径：累计独立用户 160+，但实际下场玩过的只有 47 人（其余为仅浏览/登录未开局）；看板另显示过一个 74（会话口径）。三个数字不等价，引用须注明口径。", 10.5, DIM, False)]],
     spacing=1.3, space_after=2)
...
rows = [("六套皮肤分层渲染", "已上线", True),
        ...
        ("真实用户数据", "已有 · 160+ 独立用户（47 人实际下场）/ 133 局 / 1,996 条事件", True)]
y = Inches(1.95)
